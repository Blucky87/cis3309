...
namespace InvoiceTotal
{
    public partial class FormInvoiceTotal : Form
    {
        ...
        //changed from constant
        private decimal SalesTaxPct = 7.75m;

        private void btnPercent_Click(object sender, EventArgs e)
        {
            //instatiate new form
            using (FormSalesTax form = new FormSalesTax())
            {
                //check if form's DialogResult is OK
                if (form.ShowDialog(this) == DialogResult.OK)
                {
                    //get new sales tax from form's tag property
                    SalesTaxPct = (decimal)form.Tag;
                    //change label
                    label6.Text = String.Format("Tax ({0}%):", SalesTaxPct);
                }
            }
        }
